...
export async function joinGroup(groupId: number, userWallet: string, pledgeUnits?: number): Promise<{ ok: true }>{
  ...
  try {
    ...
    await client.query('COMMIT');
    // recompute asynchronously but ensure consistency for immediate caller
...
export async function lockGroup(groupId: number): Promise<{
  ...
  try {
    ...
    // Momentum bonus: if tier achieved within first 25% of window add +2%
    ...
    for (const m of members.rows) {
      ...
      // Prepare qr payload compatible with existing redemption flows
