// Mint Discount cNFT - Modified from 3_mintCNFT.ts
...
const sampleDiscounts: DiscountCoupon[] = [
  {
...
const createDiscountMetadata = (discount: DiscountCoupon) => {
  return {
    name: discount.title,
    symbol: "DEAL",
...
const run = async () => {
  try {
    ...
    console.log("Reading merkle tree from:", merkleTreeTxt);
    
    // Wait a bit for the merkle tree to be fully confirmed
    console.log("Waiting for merkle tree to be confirmed...");
    await new Promise(resolve => setTimeout(resolve, 5000));
    
    // Retry logic for fetching merkle tree
    let merkleTreeAccount;
    let retries = 0;
    const maxRetries = 3;
    
    while (retries < maxRetries) {
      try {
        merkleTreeAccount = await fetchMerkleTree(umi, publicKey(merkleTreeTxt));
        console.log("merkleTreeAccount:", merkleTreeAccount.publicKey);
        break;
      } catch (error) {
        retries++;
        console.log(`Attempt ${retries} failed, retrying in 3 seconds...`);
    ...
    console.log(`\n🎯 Minting ${sampleDiscounts.length} discount coupons to ${data.length} addresses...\n`);

    // Mint each discount coupon
    for (let i = 0; i < sampleDiscounts.length; i++) {
      const discount = sampleDiscounts[i];
      console.log(`\n📋 Minting: ${discount.title}`);
      ...
      // Save metadata to file (in production, upload to IPFS)
      fs.writeFileSync(
      ...
      for (let j = 0; j < data.length; j++) {
        ...
        // Save mint info
        fs.writeFileSync(
          `./data/discount-${discount.redemptionCode}-mint-${j}.txt`,
    ...
    console.log('🎉 All discount coupons minted successfully!');
    console.log('📊 Summary:');
    console.log(`   - ${sampleDiscounts.length} discount types`);
    console.log(`   - ${data.length} recipients per discount`);
    console.log(`   - ${sampleDiscounts.length * data.length} total cNFTs minted`);
